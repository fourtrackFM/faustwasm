...
> extends (globalThis.AudioWorkletNode || null) {
  ...
  constructor(
    ...
    const startTime = options.processorOptions?.startTime;
    ...
    // Store start time after super() call
  ...
  protected handleMessageAux = (e: MessageEvent) => {
    ...
    } else if (e.data.type === "processingStarted") {
      console.log(`Processing started at time: ${e.data.time}`);
      // Dispatch event for processingStarted
      this.dispatchEvent(
        new CustomEvent("processingStarted", { detail: { time: e.data.time } })
      );
    }
  };
  ...
  /**
   * Set a new start time for the processor (relative to AudioContext time)
   * @param time The time in seconds when processing should begin
   */
  setStartTime(time: number) {
    this.startTime = time;
    this.port.postMessage({ type: "setStartTime", data: { time } });
  }

  /**
   * Get the current start time for the processor
