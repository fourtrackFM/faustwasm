...
export interface FaustAudioWorkletProcessorOptions {
  name: string;
  sampleSize: number;
  // Start time in seconds (relative to AudioContext time)
  startTime?: number;
  // for WAMs
  moduleId?: string;
  instanceId?: string;
...
const getFaustAudioWorkletProcessor = <Poly extends boolean = false>(
  ...
  const analysePolyParameters = (
    ...
    const isPolyReserved =
      "address" in item && !!polyKeywords.find((k) => item.address.endsWith(k));
  ...
  > extends AudioWorkletProcessor {
    ...
    constructor(options: FaustAudioWorkletNodeOptions<Poly>) {
      ...
      }); // Initialize startTime from options or default to immediate start
      this.startTime =
        options.processorOptions.startTime ??
        (globalThis as unknown as AudioWorkletGlobalScope).currentTime;
    ...
    process(
      ...
      const currentTime = (globalThis as unknown as AudioWorkletGlobalScope)
        .currentTime;
      if (!this.processingStarted && currentTime < this.startTime) {
  ...
  class FaustPolyAudioWorkletProcessor extends FaustAudioWorkletProcessor<true> {
    constructor(options: FaustPolyAudioWorkletNodeOptions) {
      ...
      const { voiceFactory, mixerModule, voices, effectFactory, sampleSize } =
